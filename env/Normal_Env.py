import numpy as np
from database.data import T_t

class normal_env():
    def __init__(self,t,T_t,T_primary,
    ...
    def EV(self, t, action):
        ...
        if connect == 1:
            ...
            elif self.EV_remain_SOC > 70:
                extra_punish = -10 - self.punish * (self.EV_remain_SOC - 70)
            else:
                extra_punish = 0
        else:
            extra_punish = 0
        ...
        terminal_penalty = 0
        if hour == 8:
            if self.EV_remain_SOC < 65:
                terminal_penalty = -(self.punish+2) * (65 - self.EV_remain_SOC)
        if connect == 1:
        ...
        rewards = anx + dam +  terminal_penalty+extra_punish
    ...
    def step(self,action,price_now):
        #动作空间
        ...
        '''功率之和'''
        P_all=P_set+MW_P_out+DIS_P_out+L_P_out+HM_P_out+HVAC_P_out+EV_P_out+ESS_P_out
        '''回报'''
        cost= price_now * P_all
        rewards=-cost+MW_reward+DIS_reward+L_reward+HM_reward+HVAC_reward+EV_reward+ESS_reward
        '''下一个状态'''
        self.t+=1
        next_state=self.get_state(price_now)
        if self.t >=36:
            done=True
        else:
            done=False

        return next_state,rewards/50,done,P_all